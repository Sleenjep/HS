...
CLICKHOUSE_JDBC_JAR_PATH = "/opt/spark/jars/clickhouse-0.4.6.jar"
...
def save_report_to_clickhouse(report_df, report_table_name, order_by_cols=None):
    ...
    if "MergeTree" in CH_TABLE_ENGINE_OPTIONS and order_by_cols:
        ...
        "MergeTree" in CH_TABLE_ENGINE_OPTIONS
        and not order_by_cols
        and "ORDER BY tuple()" not in CH_TABLE_ENGINE_OPTIONS
    ):
...
def generate_reports(spark):
    ...
    save_report_to_clickhouse(
        report_product_feedback_summary,
    ...
    save_report_to_clickhouse(
        report_yearly_sales_trends, "mart_yearly_sales_trends", order_by_cols=["year"]
    )
    ...
    if product_sales_and_rating_df.count() > 1:
        ...
        print(
            f"Correlation between product rating and sales volume: {correlation_value}"
        )
    else:
        correlation_value = None
        print(
            "Not enough data points to calculate correlation between rating and sales volume."
        )
    ...
    save_report_to_clickhouse(
        correlation_df, "mart_rating_sales_correlation", order_by_cols=[]
    )
...
if __name__ == "__main__":
    spark_session = initialize_spark_session(
        APP_NAME, POSTGRES_JDBC_JAR_PATH, CLICKHOUSE_JDBC_JAR_PATH
    )
    try:
