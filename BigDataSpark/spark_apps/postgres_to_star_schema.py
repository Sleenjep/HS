...
def process_dimension_table(
    source_df, id_col, order_col, attributes_map, target_table_name
):
    """
    ...
    processed_dim_df = (
        dim_df.withColumn("rn", row_number().over(window_spec))
        .where(col("rn") == 1)
        .drop("rn", order_col)
    )
...
def run_etl():
    ...
    source_dataframe = load_dataframe_from_postgres(
        spark, DB_CONNECTION_URL, RAW_DATA_TABLE, DB_PROPERTIES
    )
    ...
    # dim_date
    dim_date_df = (
        source_dataframe.select(col("sale_date"))
    ...
    save_dataframe_to_postgres(
        dim_date_df, DB_CONNECTION_URL, "dim_date", JDBC_WRITE_MODE, DB_PROPERTIES
    )

    # load dimension tables back to get their generated surrogate keys (_sk)
    customer_dim_sk = load_dataframe_from_postgres(
        spark, DB_CONNECTION_URL, "dim_customer", DB_PROPERTIES
    ).select("customer_sk", "customer_id")
    seller_dim_sk = load_dataframe_from_postgres(
        spark, DB_CONNECTION_URL, "dim_seller", DB_PROPERTIES
    ).select("seller_sk", "seller_id")
    product_dim_sk = load_dataframe_from_postgres(
        spark, DB_CONNECTION_URL, "dim_product", DB_PROPERTIES
    ).select("product_sk", "product_id")
    store_dim_sk = load_dataframe_from_postgres(
        spark, DB_CONNECTION_URL, "dim_store", DB_PROPERTIES
    ).select("store_sk", "name")
    supplier_dim_sk = load_dataframe_from_postgres(
        spark, DB_CONNECTION_URL, "dim_supplier", DB_PROPERTIES
    ).select("supplier_sk", "name")
    date_dim_sk = load_dataframe_from_postgres(
        spark, DB_CONNECTION_URL, "dim_date", DB_PROPERTIES
    ).select("date_sk", "sale_date")

    # process and load Fact Table
    fact_sales_df = (
        source_dataframe.join(
            date_dim_sk, source_dataframe.sale_date == date_dim_sk.sale_date, "inner"
        )
        .join(
            customer_dim_sk,
            source_dataframe.sale_customer_id == customer_dim_sk.customer_id,
            "inner",
        )
        .join(
            seller_dim_sk,
            source_dataframe.sale_seller_id == seller_dim_sk.seller_id,
            "inner",
        )
        .join(
            product_dim_sk,
            source_dataframe.sale_product_id == product_dim_sk.product_id,
            "inner",
        )
        .join(store_dim_sk, source_dataframe.store_name == store_dim_sk.name, "inner")
        .join(
            supplier_dim_sk,
            source_dataframe.supplier_name == supplier_dim_sk.name,
            "inner",
        )
        .select(
            date_dim_sk.date_sk,
    ...
    save_dataframe_to_postgres(
        fact_sales_df, DB_CONNECTION_URL, "fact_sales", JDBC_WRITE_MODE, DB_PROPERTIES
    )
